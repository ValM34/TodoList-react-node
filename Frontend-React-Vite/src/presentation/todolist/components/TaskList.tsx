            ...
            <React.Fragment key={task.id}>
              {!task.isDone ? (
                <Col span={8}>
                  <Card
                    title={
                      <a href={`./task/update/${task.id}`}>{task.title}</a>
                    }
                    bordered={false}
                    className={`card-task ${darkMode.className}`}
                    style={{ color: darkMode.textColorPrimary }}
                  >
        ...
        <Row gutter={[16, 16]}>
          {tasks.map((task: TaskModel) => (
            <React.Fragment key={task.id}>
              <Col span={8} key={task.id}>
                <Card
                  title={<a href={`./task/update/${task.id}`}>{task.title}</a>}
                  bordered={false}
                  className={`card-task ${darkMode.className}`}
                  style={{ color: darkMode.textColorPrimary }}
                >
                  ...
                  <TaskDeleteButton taskId={task.id} removeTask={removeTask} />
                </Card>
              </Col>
            </React.Fragment>
          ))}
